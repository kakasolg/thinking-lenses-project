...
def mermaid_html(mermaid_string):
    """
    Mermaid 다이어그램을 HTML iframe으로 렌더링하는 함수
    """
    html = f"""
    <div class="mermaid">
    {mermaid_string}
...
def generate_hexagram_mermaid(original_hex, target_hex, changing_lines_pos, session_history=None):
    """
    괘 변화의 흐름을 Mermaid 다이어그램으로 생성 (HTML iframe용)
    """
    if session_history is None:
        session_history = []
    
    # 현재 괘 정보
    original_data = HEXAGRAM_DATA[original_hex]
    target_data = HEXAGRAM_DATA[target_hex]
    
    # MongoDB에서 추가 정보 가져오기
    original_db_data = get_hexagram_info_from_db(original_hex)
    target_db_data = get_hexagram_info_from_db(target_hex)
    
    # 라벨 생성 (괘 번호 + 이름 + 심볼) - HTML 특수문자 처리
    original_label = f"{original_hex}번<br/>{original_data['name']}<br/>{original_data['image']}"
    target_label = f"{target_hex}번<br/>{target_data['name']}<br/>{target_data['image']}"
    
    # 정신모델 정보 추가 (MongoDB 데이터가 있는 경우)
    if original_db_data and 'mentalModels' in original_db_data:
        original_label += f"<br/>({original_db_data['mentalModels']})"
    if target_db_data and 'mentalModels' in target_db_data:
        target_label += f"<br/>({target_db_data['mentalModels']})"
    
    # Mermaid 코드 시작 (HTML용이므로 백틱 없이)
    changing_text = f"변효: {', '.join(map(str, changing_lines_pos))}" if changing_lines_pos else "변효 없음"
    
    mermaid_string = f"""
graph LR
    A["{original_label}"] -->|{changing_text}| B["{target_label}"]
    
    classDef originalClass fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef targetClass fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
    class A originalClass
    class B targetClass
    """
    
    return mermaid_string

def generate_journey_mermaid(session_history):
    """
    전체 점괘 여정을 Mermaid 다이어그램으로 생성 (HTML iframe용)
    """
    ...
    for i, (original, target, changes) in enumerate(session_history):
        original_data = HEXAGRAM_DATA[original]
        target_data = HEXAGRAM_DATA[target]
        
        original_label = f"{original}번<br/>{original_data['name']}"
        target_label = f"{target}번<br/>{target_data['name']}"
        
        if i == 0:
            # 첫 번째 점괘
            changing_text = f"변효: {', '.join(map(str, changes))}" if changes else "변효 없음"
            mermaid_string += f'    H{i}["{original_label}"] -->|{changing_text}| H{i}T["{target_label}"]\n'
        else:
            # 연속 점괘 (이전 지괘가 새로운 본괘)
            changing_text = f"변효: {', '.join(map(str, changes))}" if changes else "변효 없음"
            mermaid_string += f'    H{i-1}T -->|{changing_text}| H{i}T["{target_label}"]\n'
    
    # 스타일링
    mermaid_string += "\n"
    mermaid_string += "    classDef journeyClass fill:#fff3e0,stroke:#e65100,stroke-width:2px\n"
    for i in range(len(session_history)):
        if i == 0:
            mermaid_string += f"    class H{i} journeyClass\n"
        mermaid_string += f"    class H{i}T journeyClass\n"
...
def cast_divination():
    ...
    """
      # 디버깅을 위한 현재 상태 로그
    current_count = st.session_state.get('divination_count', 0)
    previous_target = st.session_state.get('previous_target_hex')
    logging.info(f"=== cast_divination 시작 ===")
    logging.info(f"현재 카운트: {current_count}")
    logging.info(f"이전 지괘: {previous_target}")
    logging.info(f"세션 상태: {dict(st.session_state)}")
    ...
    if previous_target is None:
        ...
        for attempt in range(max_attempts):
            ...
            if original_hex_num is not None and target_hex_num is not None:
                ...
                # 점괘 히스토리 초기화 및 저장
                if 'divination_history' not in st.session_state:
                    st.session_state.divination_history = []
                st.session_state.divination_history = [(original_hex_num, target_hex_num, changing_lines_pos)]
                
                logging.info(f"첫 점괘 생성 성공 (시도 {attempt + 1}): 본괘={original_hex_num}, 지괘={target_hex_num}, 변효={changing_lines_pos}")
                logging.info(f"세션 상태 업데이트됨 - previous_target_hex: {target_hex_num}, divination_count: 1")
        ...
        original_hex_num = previous_target
        original_lines = HEXAGRAM_DATA[original_hex_num]["lines"].copy()
        
        new_count = current_count + 1
        logging.info(f"연속 점괘 {new_count}번째 - 이전 지괘 {original_hex_num}번을 본괘로 사용")
        ...
        if target_hex_num is None:
            ...
            logging.warning(f"유효하지 않은 지괘 조합 - 랜덤 지괘 {target_hex_num} 사용")        # 세션 상태 업데이트
        st.session_state.divination_count = new_count
        st.session_state.previous_target_hex = target_hex_num
        
        # 점괘 히스토리에 추가
        if 'divination_history' not in st.session_state:
            st.session_state.divination_history = []
        st.session_state.divination_history.append((original_hex_num, target_hex_num, changing_lines_pos))
        
        logging.info(f"연속 점괘 성공: 본괘={original_hex_num} → 지괘={target_hex_num}, 변효={changing_lines_pos}")
        logging.info(f"세션 상태 업데이트됨 - previous_target_hex: {target_hex_num}, divination_count: {new_count}")
        return original_hex_num, target_hex_num, changing_lines_pos
...
if current_count == 0:
    ...
    button_text = f"🔄 다음 점괘 뽑기 ({current_count + 1}번째)"
...
if st.session_state.result:
    ...
    if original_hex is None or target_hex is None:
        ...
        current_count = st.session_state.get('divination_count', 1)
        if current_count > 1:
            st.info(f"🔗 **연속적 흐름**: 이전 {current_count-1}번째의 지괘가 이번 {current_count}번째의 본괘({original_hex}번)가 되었습니다.")
        ...
        st.markdown("---")
          # 🌟 Mermaid 시각화 섹션 추가
        st.subheader("🗺️ 점괘 변화 흐름도")
          # 현재 점괘 흐름 다이어그램 (HTML iframe으로 렌더링)
        current_mermaid = generate_hexagram_mermaid(original_hex, target_hex, changing_lines)
        if current_mermaid:
            components.html(mermaid_html(current_mermaid), height=300, scrolling=False)
        
        # 전체 여정 다이어그램 (2번째 점괘부터 표시)
        if st.session_state.get('divination_count', 0) > 1 and 'divination_history' in st.session_state:
            st.subheader("🛤️ 전체 점괘 여정")
            journey_mermaid = generate_journey_mermaid(st.session_state.divination_history)
            if journey_mermaid:
                # 여정이 길수록 높이 증가
                journey_height = min(600, 200 + len(st.session_state.divination_history) * 50)
                components.html(mermaid_html(journey_mermaid), height=journey_height, scrolling=True)
            
            # 여정 요약
            history = st.session_state.divination_history
            st.info(f"📊 **여정 요약**: {len(history)}단계의 연속적 흐름 | "
                   f"시작: {history[0][0]}번 → 현재: {history[-1][1]}번")
    ...
    **MongoDB 연결이 필요한 이유:**
    ...
    - AI 해석의 정확도와 깊이 향상    - 각 괘의 핵심관점과 실무적 적용 방법 제공
